...
void Sortlist(SLinkNode *&L)
{
	int i,j;
	SLinkNode *p,*q;			//p指向头结点
	int len = GetLength(L);
	
	for(i = 0,p = L->next;i < len;i++,p = p->next)
...
void FxxkDelRelist(SLinkNode *&L)
{
	//暴力法 
...
void DelOrdReNum(SLinkNode *&L)
{
	SLinkNode *q=L->next,*p;
